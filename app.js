...
(() => {
  ...
  function renderNav(state) {
    ...
    const links = [
      ...
      // Character creation page for players to build a character profile
  ...
  /**
   * Render the character creator page. This view allows players to
   * select a race and class from the D&D 5e API and view basic
   * information about their selection. For brevity this implementation
   * does not persist characters to localStorage, but it provides a
   * foundation for future expansion. Feel free to enhance it with
   * ability score editing and saving characters to a list.
   */
  function renderCharacterCreator(container, state) {
    ...
    container.innerHTML = `
      ...
      <div id="charCreator">
        <div>
          <label for="raceSelect"><strong>Race</strong></label><br/>
          <select id="raceSelect"><option value="">Loading races...</option></select>
        </div>
        <div style="margin-top:1rem;">
          <label for="classSelect"><strong>Class</strong></label><br/>
          <select id="classSelect"><option value="">Loading classes...</option></select>
        </div>
        <div id="raceDetails" style="margin-top:1rem;"></div>
        <div id="classDetails" style="margin-top:1rem;"></div>
      </div>
    `;
    ...
    // Fetch list of races and populate the dropdown
    ...
    // Fetch list of classes and populate the dropdown
    ...
    // Handler for race selection to fetch details
    raceSelect.onchange = () => {
      ...
      raceDetailsDiv.textContent = 'Loading race details...';
      fetch(`https://www.dnd5eapi.co/api/races/${val}`)
        ...
        .then((data) => {
          ...
          html += `<p><strong>Size:</strong> ${data.size_description}</p>`;
          if (data.ability_bonuses && data.ability_bonuses.length > 0) {
            html += '<p><strong>Ability Bonuses:</strong> ' +
              data.ability_bonuses.map((b) => `${b.ability_score.name} +${b.bonus}`).join(', ') +
              '</p>';
          }
          raceDetailsDiv.innerHTML = html;
        })
        .catch((err) => {
          raceDetailsDiv.textContent = 'Error loading race details.';
          console.error(err);
        });
    };
    // Handler for class selection to fetch details
    classSelect.onchange = () => {
      const val = classSelect.value;
      if (!val) {
        classDetailsDiv.textContent = '';
        return;
      }
      classDetailsDiv.textContent = 'Loading class details...';
      fetch(`https://www.dnd5eapi.co/api/classes/${val}`)
        ...
        .then((data) => {
          ...
          html += '</p>';
          classDetailsDiv.innerHTML = html;
        })
        .catch((err) => {
          classDetailsDiv.textContent = 'Error loading class details.';
          console.error(err);
        });
    };
  }
